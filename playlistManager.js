				...
				if (err)
					...
					for (var i in data) {
						...
						uris.push({uri:uri,service:service});
										...
										// Play first track in added list.
										// Index is then the lenght of the queue before adding.
										var indexToBePlayed = self.commandRouter.volumioGetQueue().length;

                    self.commandRouter.addQueueItems(uris)
                        .then(function()
                        {
                            self.commandRouter.volumioPlay(indexToBePlayed);
                            defer.resolve();
                        })
                        .fail(function()
                        {
